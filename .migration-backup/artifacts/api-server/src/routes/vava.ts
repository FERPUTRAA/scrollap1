...
vavaRouter.get("/vava/live-sessions", async (_req: Request, res: Response) => {
  try {
    // Try multiple endpoints in parallel — VAVA API version varies; v2 now returns null
    // so we try v1 and list endpoints as fallback to maximise session discovery.
...
vavaRouter.post("/vava/session", async (_req: Request, res: Response) => {
  ...
  try {
    ...
    if (d?.channel && (d?.agoraToken || d?.authToken)) {
      ...
      // Generate a cert-signed anonymous token (uid=0) so frontend can
      // join as a silent spectator without using the credential's uid.
      const serverToken = generateToken(AGORA_APP_ID, AGORA_APP_CERTIFICATE, d.channel, 0);
      // Cache the matched session so /api/vava/call-sessions also exposes it
      pruneCallSessions();
      ...
      return res.json({
        ...
        channel: d.channel,
        token: vavaToken,            // VAVA original token (tied to credentialUid)
        serverToken,                 // Our cert-signed token (uid=0)
        uid,                         // uid for VAVA token; use 0 with serverToken
...
function extractAgoraCredentials(obj: unknown, depth = 0): { channel: string; token: string } | null {
  ...
  // Extract channel from all known field names
  const ch = (
    o.channel ?? o.channelName ?? o.channelId ?? o.roomId ?? o.agoraChannel ?? o.rtcChannel
  ) as string | undefined;

  if (typeof ch === "string" && ch.length > 0) {
    // Try every known Agora/VAVA token field name in priority order
    const tok = (
      o.agoraToken ?? o.rtcToken ?? o.roomToken ?? o.videoToken ??
      o.authToken ?? o.token ?? o.chatToken ?? o.key ?? o.accessToken
    ) as string | undefined;
    // Accept if it looks like a real Agora token (starts 006/007) or is long enough
