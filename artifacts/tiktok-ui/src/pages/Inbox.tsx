...
const EXTENDED_NOTIFICATIONS = [
  ...MOCK_NOTIFICATIONS,
  { id: "n4", user: "jenny_creative", action: "liked your video", time: "2d", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=jenny", type: "like" },
  { id: "n5", user: "dance_king99", action: "started following you", time: "3d", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=dancer", type: "follow" },
  { id: "n6", user: "foodie_world", action: "commented: So delicious!", time: "3d", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=food", type: "comment" },
  { id: "n7", user: "travel_adventures", action: "liked your video", time: "5d", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=adventures", type: "like" },
].map((n) => ({ type: "like", ...n }));
        ...
        {activeTab === "activity" ? (
          <div className="divide-y divide-white/5">
            {EXTENDED_NOTIFICATIONS.map((notif) => (
              <div key={notif.id} data-testid={`notification-item-${notif.id}`} className="flex items-center gap-3 px-4 py-3">
                <div className="relative flex-shrink-0">
                  <img
                    src={notif.avatar}
                    alt={notif.user}
                    className="w-11 h-11 rounded-full bg-gray-700 object-cover"
                    onError={(e) => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${notif.user}&background=333&color=fff&size=44`; }}
                  />
                ...
                <div className="w-10 h-12 rounded-sm flex-shrink-0" style={{ background: "linear-gradient(135deg, #FF9A9E, #FECFEF)" }} />
              </div>
            ))}
          </div>
        ) : (
          <div className="divide-y divide-white/5">
            {FOLLOWERS.map((f) => {
              ...
              return (
                <div key={f.id} data-testid={`follower-item-${f.id}`} className="flex items-center gap-3 px-4 py-3">
                  <img
                    src={f.avatar}
                    alt={f.user}
                    className="w-11 h-11 rounded-full bg-gray-700 object-cover"
                    onError={(e) => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${f.user}&background=333&color=fff&size=44`; }}
                  />
                  ...
                  <button
                    ...
                    onClick={() => setFollowed((prev) => { const next = new Set(prev); if (next.has(f.id)) next.delete(f.id); else next.add(f.id); return next; })}
                    className={`text-sm font-semibold px-4 py-1.5 rounded-md transition-colors ${isFollowing ? "bg-transparent border border-white/30 text-white" : "bg-[#EE1D52] text-white"}`}
                  >
