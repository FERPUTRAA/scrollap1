    ...
    <div className="h-full w-full bg-black overflow-y-auto no-scrollbar">
      ...
      <div className="flex flex-col items-center px-4 pb-4">
        <div className="relative mb-3">
          <div className="w-20 h-20 rounded-full overflow-hidden" style={{ background: "linear-gradient(135deg, #EE1D52, #69C9D0)" }}>
            <img
              src="https://api.dicebear.com/7.x/avataaars/svg?seed=myprofile"
              alt="avatar"
              className="w-full h-full object-cover"
              onError={(e) => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=Me&background=ee1d52&color=fff&size=80`; }}
            />
          </div>
        </div>
        ...
        <div className="flex gap-2 w-full">
          <button data-testid="button-edit-profile" className="flex-1 py-2 rounded-md bg-[#1a1a1a] text-white text-sm font-semibold border border-white/10">
            Edit profile
          </button>
          <button data-testid="button-share-profile" className="w-10 h-9 rounded-md bg-[#1a1a1a] flex items-center justify-center border border-white/10">
      ...
      <div className="flex border-b border-white/10 sticky top-0 bg-black z-10">
        <button
          ...
          className={`flex-1 py-2.5 flex items-center justify-center gap-1.5 border-b-2 -mb-[1px] transition-colors ${activeTab === "videos" ? "border-white text-white" : "border-transparent text-white/40"}`}
        >
          <Grid3x3 size={18} />
        </button>
        <button
          ...
          className={`flex-1 py-2.5 flex items-center justify-center gap-1.5 border-b-2 -mb-[1px] transition-colors ${activeTab === "liked" ? "border-white text-white" : "border-transparent text-white/40"}`}
        >
      ...
      <div className="grid grid-cols-3 gap-[1px] pb-20">
        {VIDEO_COLORS.map((color, i) => (
          ...
          >
            <div className="absolute bottom-0 left-0 right-0 p-1" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.7), transparent)" }}>
